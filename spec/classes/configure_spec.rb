...
describe 'threatstack::v1::configure' do
  ...
  context 'on Debian' do
    ...
    let(:pre_condition) { "class { 'threatstack::v1': deploy_key => '#{deploy_key}', feature_plan => '#{feature_plan}', ts_hostname => '#{ts_hostname}', ruleset => ['Default Ruleset', 'Service Ruleset'] }" }
