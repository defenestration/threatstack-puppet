...
#
class threatstackv1::configure {

  $rulesets     = $::threatstackv1::ruleset # bring value into scope.
  $ruleset_args = inline_template("<% @rulesets.each do |ruleset| -%> --ruleset='<%= ruleset %>'<% end -%>")

  $feature_plan_arg = $::threatstackv1::feature_plan ? {
    investigate => 'agent_type="i"',
    monitor     => 'agent_type="m"',
    legacy      => 'agent_type="i"'
  }

  $full_config_args_list = delete_undef_values([$::threatstackv1::agent_config_args, $feature_plan_arg])
  $full_config_args = join($full_config_args_list, ' ')

  exec { 'threatstack-agent-setup':
    command   => "/opt/threatstack/bin/cloudsight setup --deploy-key='${::threatstackv1::deploy_key}' --hostname='${::threatstackv1::ts_hostname}' ${ruleset_args} ${::threatstackv1::agent_extra_args}",
    subscribe => Package[$threatstackv1::ts_package],
  ...
  exec { 'threatstack-agent-configure':
    ...
    refreshonly => true,
    path        => ['/bin', '/usr/bin'],
    notify      => Class['threatstackv1::service']
  }
